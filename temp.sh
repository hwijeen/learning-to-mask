#!/bin/bash
export task=$1
declare -A lr=(["mrpc"]=1e-5 ["rte"]=1e-4 ["cola"]=1e-5 ["sst2"]=1e-5 ["qnli"]=1e-4 ["qqp"]=1e-5 ["stsb"]=1e-5 ["mnli"]=1e-4 )
...
CUDA_VISIBLE_DEVICES=0 python run_glue.py \
  ...
  --num_train_epochs 10 \
  --output_dir outs/mask_cloze/$task_${lr[$task]} \
  --logging_steps 10 \
  --report_to tensorboard \
  --logging_dir logs/mask_cloze/$task_${lr[$task]} \
